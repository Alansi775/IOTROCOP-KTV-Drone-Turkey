...
Monitors serial data and transmits to:
  ...
  3. Flutter UI via UDP Bridge (NEW)
"""
...
class FlutterBridge:
    """
    Real-time data bridge for Flutter UI
    Publishes joystick + power data via UDP
    ...
    def __init__(self, target_ip: str = "127.0.0.1", target_port: int = 5657):
        ...
        print(f"{Colors.FLUTTER}🎮 Flutter Bridge initialized{Colors.RESET}")
    ...
    def publish_joystick_data(self, packet: DronePacket) -> bool:
        """
        Publish joystick + power data to Flutter
...
class DroneMonitor:
    ...
    def format_switch_states(self, switch_bits: int) -> str:
        ...
        return format(switch_bits, '014b')
    ...
    def display_packet(self, packet: DronePacket):
        ...
        output += f"{Colors.NETWORK}📡{Colors.RESET} {flutter_icon}\n"
        ...
        output += f"{Colors.SWITCH}SW:{Colors.RESET} {self.format_switch_states(packet.switch_states)}"
    ...
    def read_serial_data(self):
        ...
        print(f"{Colors.BOLD}{Colors.HEADER}  STM32 DRONE MONITOR v3.0 - FLUTTER EDITION{Colors.RESET}")
        ...
        if self.enable_flutter:
            print(f"{Colors.FLUTTER}🎮 Flutter Bridge: ENABLED{Colors.RESET}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='STM32 Drone Monitor with Network + Flutter Bridge',
...
Examples:
  ...
  %(prog)s --target 192.168.100.3 --port 6666       # Custom network target
    ...
    print("╔════════════════════════════════════════════════════════════════════╗")
    print("║     STM32 DRONE MONITOR v3.0 - FLUTTER EDITION                    ║")
    print("║     Serial Monitor + Network Transmitter + Flutter Bridge          ║")
    print("╚════════════════════════════════════════════════════════════════════╝")
